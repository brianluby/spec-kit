...
class MarkdownIntegrationTests:
    ...
    def _expected_files(self, script_variant: str) -> list[str]:
        ...
        if script_variant == "sh":
            for name in ["check-prerequisites.sh", "common.sh", "configure-worktree.sh",
                         "create-new-feature.sh", "setup-plan.sh", "setup-tasks.sh"]:
                files.append(f".specify/scripts/bash/{name}")
        else:
            for name in ["check-prerequisites.ps1", "common.ps1", "configure-worktree.ps1",
                         "create-new-feature.ps1", "setup-plan.ps1", "setup-tasks.ps1"]:
                files.append(f".specify/scripts/powershell/{name}")

        for name in ["agent-file-template.md", "ar-template.md", "checklist-template.md",
                     "constitution-template.md", "plan-template.md", "prd-template.md",
                     "roadmap-template-v1.md", "sec-template.md", "spec-template.md",
                     "tasks-template.md", "vision-template-v1.md"]:
