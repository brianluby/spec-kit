...
# Read a value from .specify/config.json (with global config fallback)
# Usage: read_config_value "key" [default_value] [config_file_path]
read_config_value() {
    local key="$1"
...
get_execution_mode() {
    ...
    case "$mode" in
        fast|balanced|detailed)
            ...
            ;;
        *)
            echo "ERROR: Unknown execution mode '$mode'. Valid values: fast, balanced, detailed." >&2
            echo "balanced"
            return 1
            ;;
    esac
