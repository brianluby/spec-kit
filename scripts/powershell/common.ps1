...
# ---------------------------------------------------------------------------
# Adaptive Execution Modes -- config readers and risk detection
# ---------------------------------------------------------------------------
...
function Get-GlobalConfigPaths {
    ...
    @(
        ...
        (Join-Path $xdgConfigHome 'specify/config.json')
    )
}

function Read-ConfigValueFromFile {
    param(
    ...
    try {
        ...
        if ($null -ne $config.PSObject.Properties[$Key]) {
...
function Read-ConfigValue {
    ...
    foreach ($candidate in $candidateFiles) {
        $value = Read-ConfigValueFromFile -Key $Key -ConfigFile $candidate
...
function Read-FeatureConfigValue {
    param(
        [Parameter(Mandatory = $true)][string]$Key,
        [string]$FeatureDir = '',
        [string]$DefaultValue = ''
    ...
    $configFile = Join-Path $FeatureDir '.feature-config.json'
    return (Read-ConfigValue -Key $Key -DefaultValue $DefaultValue -ConfigFile $configFile)
}

function Detect-RiskTriggers {
    ...
    $keywords = @(
        'auth', 'authentication', 'authorization', 'payment', 'billing', 'pii',
        'personal data', 'personal information', 'external api', 'third-party',
        'third party', 'delete', 'destroy', 'drop', 'admin', 'compliance',
        'regulation', 'gdpr', 'hipaa', 'encryption', 'secret', 'credential',
        'token', 'password'
    )

    $content = Get-Content -LiteralPath $SpecFile -Raw
    $matched = New-Object System.Collections.Generic.List[string]

    foreach ($keyword in $keywords) {
        $canonical = $keyword.Replace(' ', '-')
        if ($keyword -match '[\s-]') {
        ...
        if ($content -match "(?i)$pattern" -and -not $matched.Contains($canonical)) {
...
function Get-ExecutionMode {
    ...
    $mode = Read-FeatureConfigValue -Key 'mode' -FeatureDir $FeatureDir -DefaultValue ''
    if (-not $mode) {
        $mode = Read-ConfigValue -Key 'defaultMode' -DefaultValue ''
    }
    if (-not $mode) {
        $mode = 'balanced'
    }

    if ($mode -in @('fast', 'balanced', 'detailed')) {
        return $mode
    }

    [Console]::Error.WriteLine("ERROR: Unknown execution mode '$mode'. Valid values: fast, balanced, detailed.")
    return 'balanced'
}
